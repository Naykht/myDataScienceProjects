...
|:-------|:-------:|:-------|:-------:|:-------:|
|[Предсказание цен на недвижимость][1]|ИАД|Необходимо реализовать алгоритмы kNN и линейного регрессии с нуля и добиться качества не больше 0.121 на тестовых данных по метрике *RMSE*.|`pandas` `sklearn` `numpy`|Завершен|
|[Предсказание стоимости автомобилей][2]|Яндекс.Практикум|Необходимо построить модель машинного обучения, которая сможет быстро и качественно подсказать клиенту рыночную стоимость автомобиля.|`pandas` `sklearn` `бустинг (catboost, lightgbm)`|Завершен|
