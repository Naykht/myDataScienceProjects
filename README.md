...
|:-------|:-------:|:-------|:-------:|:-------:|
|[Предсказание цен на недвижимость][1]|ИАД|Необходимо реализовать алгоритмы kNN и линейного регрессии с нуля и добиться качества не больше 0.121 на тестовых данных по метрике *RMSE*.|`pandas` `sklearn` `numpy` `визуализация (matplotlib, seaborn)`|Завершен|
|[Предсказание стоимости автомобилей][2]|Яндекс.Практикум|Необходимо построить модель машинного обучения, которая сможет быстро и качественно подсказать клиенту рыночную стоимость автомобиля.|`pandas` `sklearn` `numpy` `бустинг (catboost, lightgbm)` `визуализация (matplotlib, seaborn)`|Завершен|
|[Бурение скважины][3]|Яндекс.Практикум|С помощью модели машинного обучения необходимо определить регион, который принесет наибольшую прибыль.|`pandas` `sklearn` `numpy` `scipy` `bootstrap` `визуализация (matplotlib, seaborn)`|`Завершен`|
