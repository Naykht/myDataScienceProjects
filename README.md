...
|Название проекта|Источник|Описание|Инструменты|Статус|
|:-------|:-------:|:-------|:-------:|:-------:|
|[Предсказание категории товара][13]|ИАД|Имеется информация об объектах 50 классов. Необходимо по новым объектам предсказать категорию товара.|`pandas` `numpy` `matplotlib`  `sklearn` `BOW` `TF-IDF` `Hasing Vectorizer` `Word Vectors` `pymystem3` `pymorphy2` `spacy`|Завершен|
